...
public final class FloatWrapper extends DataClayJavaWrapper {
	...
	public void serialize(final DataClayByteBuffer dcBuffer, final boolean ignoreUserTypes,
		...
		if (DataClaySerializationLib.DEBUG_ENABLED) { 
			DataClaySerializationLib.LOGGER.debug("[Serialization] --> Serialized Float: data=" + floatObject + ", writerIndex=" + dcBuffer.writerIndex());
		}
	}
	...
	public void deserialize(final DataClayByteBuffer dcBuffer,
		...
		if (DataClayDeserializationLib.DEBUG_ENABLED) { 
			DataClayDeserializationLib.LOGGER.debug("[Deserialization] --> Float deserialized: data="+  floatObject + ", readerindex=" + dcBuffer.readerIndex());
		}
	}
